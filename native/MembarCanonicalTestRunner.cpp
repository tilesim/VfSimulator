...
int main(int argc, char **argv) {
  try {
    ...
    std::ifstream input(args.tracePath);
    if (!input) throw std::runtime_error("cannot open canonical input");
    std::ostringstream text;
    text << input.rdbuf();
    const auto vfInfo = decodeCanonicalVfInfoFixture(json::parseString(text.str()));
    const auto result =
