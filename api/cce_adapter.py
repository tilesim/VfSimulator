        ...
        scopes.append(
            CCEVFScope(
                ...
                param_dtypes={**_parse_ub_param_dtypes(match.group("params")),
                              **_parse_scalar_param_dtypes(match.group("params"))},
...
def _parse_scalar_param_dtypes(params: str) -> Dict[str, str]:
    dtypes: Dict[str, str] = {}
    for raw in _split_args(params):
        cleaned = raw.strip()
        if "__ubuf__" in cleaned:
            continue
        match = re.fullmatch(
            r"(?:(?:const|volatile)\s+)*"
        ...
        if match:
            dtypes[match.group(2)] = _c_scalar_dtype_to_form(match.group(1))
    return dtypes
