...
日期：2026-09-17。当前分支：`shq-inorder-experiment`。来源：master `a4d355f`。
...
- Python unittest：229/229 通过，启用本分支 Native 对照。
- Native CTest：5/6。
- A5 AABBCC U1/U2/U4/U8 POST_UPDATE：189/185/276/290 cycles；
  显式 offset：189/194/198/189 cycles。Python/Native 逐项一致。
- 同指针、独立指针、显式 offset 三种积压 probe 均为 157 cycles。
  同指针 IDU dispatch 为 26..41，13 条地址阻塞记录；另外两种无地址阻塞。
  三种模式均允许积压后的 16 条 load 在 8 个周期中双发。

- 保留旧 VFInfo 兼容入口；本轮新增脚本显式调用 `run_canonical_vf_info`。
- Native 仍未实现 align-state lowering，跨语言用例不覆盖 VSTUS/VSTAS。
- Native 对照 runner 为 `vfsim_address_canonical_test_runner`。

- Native smoke 仍有已知 balanced-reserve 断言失败，上一轮已在 `82081d8` 独立复现。
  本轮其余五项与地址测试通过，不改变实验调度策略或断言来隐藏失败。
