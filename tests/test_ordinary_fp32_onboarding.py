...
class OrdinaryFp32OnboardingTest(unittest.TestCase):
    ...
    def test_cce_canonical_and_python_timing(self):
        for op, (latency, _, _) in OPS.items():
            with self.subTest(op=op), tempfile.TemporaryDirectory() as name:
                ...
                vf = InputAPI.load_cce_canonical(source, 'ordinary_probe')
                ...
                result = CoreVfCostModel(base_dir=ROOT, out_dir=root/'python').run_canonical_vf_info(vf)
    ...
    def test_python_native_straightline_cycle_parity(self):
        for op in OPS:
            with self.subTest(op=op), tempfile.TemporaryDirectory() as name:
                root = Path(name)
                source = root/'probe.cce'
                ...
                # This branch's native CLI reads legacy program JSON. The probe
                # is straight-line SSA, so its operands need no loop conversion.
                trace.write_text(json.dumps(CoreLoweringPass().lower(vf)))
                result = CoreVfCostModel(base_dir=ROOT, out_dir=root/'python').run_canonical_vf_info(vf)
